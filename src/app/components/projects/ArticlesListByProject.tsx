...
const ArticlesListByProject = ({ projectId } ) => {
    ...
    useEffect(() => {
        const getArticlesLength = async () => {
            try {
                const {data} = await axios.get('/api/project/get_length_articles_by_project', {
                    params: {
        ...
        const getArticles = async () => {
            try {
                const {data} = await axios.get('/api/project/get_articles_by_project', {
                    params: {
                        uid: projectId.projectId,
                        page,
                        take,
        ...
        <ul className="articles-list">
            {
                articles?.map(article => (
                    <li>
                        ...
                        {popUpDelete === article.uid &&< div className="pop-up-delete" style={{background: '#000'}}>
...
export default ArticlesListByProject
